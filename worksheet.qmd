...
```{r}
votes <- votes |>
    mutate(dem_share = d_votes / (d_votes + r_votes))

mean_median_score <- mean(votes$dem_share) - median(votes$dem_share)
```
...
votes_waste <- votes |>
    mutate(
        ...
        wasted_D = if_else(winner == "D", d_votes - 50, d_votes),
        wasted_R = if_else(winner == "R", r_votes - 50, r_votes)
    )

efficiency_gap <- (sum(votes_waste$wasted_D) - sum(votes_waste$wasted_R)) / sum(votes_waste$d_votes + votes_waste$r_votes)
```